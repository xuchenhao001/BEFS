...
def average_local_w(local_uuid, from_ip, w_compressed):
    ...
    if model_store.local_models_add_count(local_uuid, utils.util.decompress_tensor(w_compressed),
        ...
        for w_local_id in model_store.local_models:
            trainer.load_model(model_store.local_models[w_local_id])
            loss, _, _, _, _ = trainer.evaluate_model_loss()
            model_store.local_models_loss[w_local_id] = loss
        w_glob = fed_lfr(model_store.local_models, model_store.local_models_loss, model_store.global_model,
