...
from models.Fed import signSGD
...
def train(uuid, w_glob, epochs):
    ...
    w_local = utils.util.extract_sign(w_local, w_glob)
    # fake attackers
...
def release_global_w(epochs):
    ...
    g_uuid = 0
    lock.release()
    w_glob = signSGD(wMap, g_train_global_model, args.server_lr)
    # update global model in the cache
