...
function releaseCerts() {
  ...
  for i in "${!PEER_ADDRS[@]}"; do
    ...
    scp networkCache.tar.gz ${HOST_USER}@${addrIN[0]}:~/BEFS/fabric-network/networkCache.tar.gz
    ssh ${HOST_USER}@${addrIN[0]} "cd ~/BEFS/fabric-network/ && tar -zxf networkCache.tar.gz"
  done
}
