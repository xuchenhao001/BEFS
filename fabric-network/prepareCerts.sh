...
function releaseCerts() {
  ...
  for i in "${!PeerAddress[@]}"; do
    ...
    scp networkCache.tar.gz ${HostUser}@${addrIN[0]}:~/BEFS/fabric-network/networkCache.tar.gz
    ssh ${HostUser}@${addrIN[0]} "cd ~/BEFS/fabric-network/ && tar -zxf networkCache.tar.gz"
  done
}
