...
function main() {
    for i in "${!MODEL_DS[@]}"; do
        ...
        if [[ ! -d "${model}-${dataset}/${scheme_name}_sgd_10" ]]; then
            echo "[`date`] ## ${scheme_name}_sgd_10 start ##"
            clean
            for i in "${!PEER_ADDRS[@]}"; do
              ...
              PYTHON_CMD="python3 -u ${scheme_name}.py --model=${model} --dataset=${dataset} --sign_sgd --server_lr=1"
            ...
            arrangeOutput ${model} ${dataset} "${scheme_name}_sgd_10"
            echo "[`date`] ## ${scheme_name}_sgd_10 done ##"
        fi
